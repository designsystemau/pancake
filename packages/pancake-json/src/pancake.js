...
module.exports.pancake = (version, modules, settings, GlobalSettings, cwd) => {
	...
	return new Promise((resolve, reject) => {
		...
		if (typeof version !== 'string') {
			reject(
				`Plugin pancake-json got a mismatch for the data that was passed to it! ${Style.yellow(
					`version`
		...
		if (SETTINGS.json.enable) {
			for (const modulePackage of modules) {
				...
				if (SETTINGS.json.content.dependencies) {
					JSONOutput[modulePackage.name].dependencies =
						modulePackage.peerDependencies;
				}
				...
				if (SETTINGS.json.content.settings) {
					JSONOutput[modulePackage.name].settings =
						modulePackage.pancake['pancake-module'];
				}
			}

			if (Object.keys(JSONOutput).length > 0) {
				const jsonPath = Path.normalize(
					`${cwd}/${SETTINGS.json.location}/${SETTINGS.json.name}.json`
				);
