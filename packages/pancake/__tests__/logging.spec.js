...
test('Log.parse - start and end ansi code is correctly added', () => {
	expect(Style.parse('TEST', '666m', '777m')).toBe(
		'\u001B[666mTEST\u001b[777m'
	);
});
...
test('function should return correct string and colour', () => {
	...
	expect(Style.magenta('test magenta')).toBe(
		'\u001B[35mtest magenta\u001b[39m'
	);
...
test('should be able to combine multiple strings of varying colours', () => {
	const test = Style.yellow(
		`yellow text ${Style.green(
			`green text ${Style.red(`red text`)} green text`
		)} yellow text`
	);

	expect(test).toBe(
		'\u001B[33myellow text \u001B[32mgreen text \u001B[31mred text\u001B[32m green text\u001B[33m yellow text\u001b[39m'
	);
});
...
test('Log.ok - Log.space should only be called the first time a Log function is run', () => {
	...
	expect(console.info.mock.calls[0][0]).toBe(
		`👍  \u001B[32mOK:\u001b[39m      \u001B[32mtest\u001b[39m`
	);
	expect(console.info.mock.calls[1][0]).toBe(
		`👍  \u001B[32mOK:\u001b[39m      \u001B[32mtest2\u001b[39m`
	);
});
