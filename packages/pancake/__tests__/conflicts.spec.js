...
const resultMinorConflict = {
	...
	dependencies: {
		'^11.5.3': ['@gov.au/testmodule3'],
		'^11.0.1': ['@gov.au/testmodule2'],
	},
};

test('Single minor conflict between pancake modules should return correct result', () => {
	expect(CheckModules(allModulesMinorConflict)).toMatchObject(
		resultMinorConflict
	);
});
...
const resultMinorConflicts = {
	...
	dependencies: {
		'^11.5.3': ['@gov.au/testmodule3'],
		'^11.6.2': ['@gov.au/testmodule2'],
	},
};

test('Multiple minor conflicts between pancake modules should return correct result', () => {
	expect(CheckModules(allModulesMinorConflicts)).toMatchObject(
		resultMinorConflicts
	);
});
...
const resultMajorConflict = {
	...
	module: '@gov.au/testmodule1',
	dependencies: {
		'^10.0.1': ['@gov.au/testmodule3'],
		'^11.0.1': ['@gov.au/testmodule2'],
	},
};

test('Single major conflict between pancake modules should return correct result', () => {
	expect(CheckModules(allModulesMajorConflict)).toMatchObject(
		resultMajorConflict
	);
});
...
const resultMajorConflicts = {
	...
	dependencies: {
		'^10.0.1': ['@gov.au/testmodule3'],
		'^11.0.1': ['@gov.au/testmodule2'],
		'^9.0.1': ['@gov.au/testmodule4'],
	},
};

test('Multiple major conflicts between pancake modules should return correct result', () => {
	expect(CheckModules(allModulesMajorConflicts)).toMatchObject(
		resultMajorConflicts
	);
});
