...
module.exports.init = (argv = process.argv) => {
	...
	SETTINGS.ignorePlugins = ARGS.ignorePlugins.length
		? ARGS.ignorePlugins
		: SETTINGS.ignorePlugins;
  ...
  \`@:                                   \`:;+#@@##@@@+;,\`              ${Style.white(
    ...
    ;#+;            \`\`\`                                               ${Style.white(
		...
		console.log(
			Style.yellow(`\n  ( ^-^)_旦\n\n`) +
				...
				`  ${Style.bold(
					`IGNORED PLUGINS`
				)} - Prevent a certain plugin(s) from being installed and run.\n` +
				`    $ ${Style.yellow(
					`pancake --ignore @gold.au/pancake-js,@gold.au/pancake-sass`
				)}\n\n` +
				`  ${Style.bold(
					`DON’T SAVE`
				)}      - Prevent pancake to save it’s settings into your package.json.\n` +
	...
	GetModules(pkgPath, SETTINGS.npmOrg)
		.catch((error) => {
			Log.error(
				`Reading all package.json files bumped into an error: ${error}`
			);
			Log.error(error);
			...
			if (allModules.length < 1) {
				...
				if (SETTINGSlocal.plugins === false || SETTINGS.plugins === false) {
					...
					allPlugins.map((plugin) => {
						...
						if (
							SETTINGSlocal.ignore.filter((ignore) => ignore === plugin)
								.length === 0 &&
							SETTINGS.ignorePlugins.filter((ignore) => ignore === plugin)
								.length === 0
						) {
							plugins.push(plugin);
						}
								...
								if (SETTINGSlocal['auto-save'] && !ARGS.nosave) {
									...
									settings.map((setting) => {
										Object.keys(setting).map((key) => {
											SETTINGSlocal[key] = Object.assign(
												setting[key],
												SETTINGSlocal[key]
											);
										});
