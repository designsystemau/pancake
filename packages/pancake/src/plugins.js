...
module.exports.InstallPlugins = (plugins, cwd) => {
	...
	return new Promise((resolve, reject) => {
		...
		if (result.installing.length > 0) {
			...
			const cacheLockStale = Spawning.sync('npm', [
				'config',
				'get',
				'cache-lock-stale',
			])
				.stdout.toString()
				.trim();
			const cacheLockWait = Spawning.sync('npm', [
				'config',
				'get',
				'cache-lock-wait',
			])
			...
			Log.verbose(
				`npm config was cache-lock-stale: ${Style.yellow(
					cacheLockStale
				)} cache-lock-wait: ${Style.yellow(cacheLockWait)}`
			);
			...
			if (hasYarn) {
				...
				Spawning.async(
					'npm',
					['install', '--no-progress', '--save', ...result.installing],
					spawnOpt
				)
					.catch((error) => {
						Loading.stop();
...
module.exports.RunPlugins = (
	...
	return new Promise((resolve, reject) => {
		...
		const allPlugins = plugins.map((plugin) => {
			...
			return plugin
				.pancake(version, allModules, SETTINGSlocal, SETTINGS, cwd) //run ’em
				.catch((error) => {
					Log.error(error);
