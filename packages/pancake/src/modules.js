	...
	return new Promise((resolve, reject) => {
		Fs.readFile(thisPath, `utf8`, (error, data) => {
			if (error) {
				...
				if (typeof packageJson.pancake === 'object') {
					//is this a pancake module?
					Log.verbose(
						`${Style.green('✔')} Identified ${Style.yellow(
							packageJson.name
						)} as pancake module`
					);
...
module.exports.GetModules = (pkgPath, npmOrgs = '') => {
	if (typeof pkgPath !== 'string' || pkgPath.length <= 0) {
		Log.error(
			`GetPackages only takes a valid path. You passed [type: ${Style.yellow(
				typeof pkgPath
			)}] "${Style.yellow(pkgPath)}"`
		);
	}
	...
	const npmOrgModules = npmOrgs.split(' ').map((npmOrg) => {
		...
		const altModulesPath = Path.normalize(
			`${pkgPath}/../node_modules/${npmOrg}/`
		);
		if (Fs.existsSync(altModulesPath)) {
			Log.verbose(
				`Also looking for pancake modules in: ${Style.yellow(altModulesPath)}`
			);
	...
	if (allModules !== undefined && allModules.length > 0) {
		Log.verbose(
			`Found the following module folders:\n${Style.yellow(
				allModules.join('\n')
			)}`
		);

		const allPackages = allModules.map((pkg) => {
			return ReadModule(pkg).catch((error) => {
				Log.error(error);
		...
		return Promise.all(allPackages).then((packages) => {
...
module.exports.GetPlugins = (allModules) => {
	...
	allModules.map((module) => {
		...
		if (module.pancake['pancake-module'] === undefined) {
			Log.error(
				`The data passed to GetPlugins is missing the "${Style.yellow(
					`pancake-module`
				)}" object.`
			);
	...
	Log.verbose(
		`Found the following plugins ${Style.yellow(
