 ...
 */
module.exports.ParseArgs = (SETTINGS, args = process.argv) => {
		...
		for (index; index < args.length; index++) {
			...
			if (allowed[arg] === undefined) {
				...
				if (command.options > 0) {
					//flag with options
					for (let i = 0; i < command.options; i++) {
						//iterating over the options by moving along in the process.argv array
						index++;

						if (args[index] === undefined) {
							Log.error(
								`There are some missings options in the commande ${Style.yellow(
									arg
								)}`
							);
						} else {
							if (typeof defaults[command.name] === 'object') {
