...
const modules = [
	{
		...
		pancake: {
			'pancake-module': {
				version: '1.0.0',
				plugins: ['@gov.au/pancake-sass', '@gov.au/pancake-js'],
				sass: {
...
const baseLocation = Path.normalize(
	`${__dirname}/../../../tests/test1/node_modules/@gov.au/`
);
const npmOrg = '@gov.au';
const resultPath = Path.normalize(
	`${__dirname}/../../../tests/test1/node_modules/@gov.au/testmodule2/lib/sass/_module.scss`
);
...
test('GetPath should return path for sass partial with multiple orgs', () => {
	expect(
		GetPath(moduleName, modules, baseLocation, '@gov.au @nsw.gov.au')
	).toBe(resultPath);
});
...
test('GetDependencies should return object of all dependencies', () => {
	expect(GetDependencies(moduleName, modules)).toMatchObject(
		ResultDependencies
	);
});
...
//--------------------------------------------------------------------------------------------------------------------------------------------------------------
const sassPath = Path.normalize(
	`${__dirname}/../../../tests/test1/node_modules/`
);
...
const Location = Path.normalize(
	`${__dirname}/../../../tests/test1/node_modules/@gov.au/testmodule2`
);

test('GenerateSass should return path to sass partial import', () => {
	expect(GenerateSass(Location, moduleName, modules, npmOrg)).toBe(
		ResultGenerateSass
	);
});
...
// Sassify function
//--------------------------------------------------------------------------------------------------------------------------------------------------------------
const pancakeVersion = require(Path.normalize(
	`${__dirname}/../../pancake/package.json`
)).version;
const pancakeSassVersion = require(Path.normalize(
	`${__dirname}/../package.json`
)).version;
const cssLocation = Path.normalize(
	`${__dirname}/../../../tests/test1/pancake/css/pancake.min.css`
);
...
const sass =
	`/*! PANCAKE v${pancakeVersion} PANCAKE-SASS v${pancakeSassVersion} */\n\n` +
