...
module.exports.pancake = (version, modules, settings, GlobalSettings, cwd) => {
	...
	return new Promise((resolve, reject) => {
		...
		if (typeof version !== 'string') {
			reject(
				`Plugin pancake-sass got a mismatch for the data that was passed to it! ${Style.yellow(
					`version`
		...
		const sassVersioningPath = Fs.existsSync(
			Path.normalize(`${cwd}/../node_modules/sass-versioning`)
		)
			? Path.normalize(
					`${cwd}/../node_modules/sass-versioning/dist/_index.scss`
			  ).replace(/\\/g, '\\\\')
			: Path.normalize(
					`${cwd}/node_modules/sass-versioning/dist/_index.scss`
			  ).replace(/\\/g, '\\\\');
		...
		const banner =
			`/*! PANCAKE v${version} PANCAKE-SASS v${Package.version} */\n\n` +
			`/*\n` +
		...
		for (const modulePackage of modules) {
			...
			let sassModulePath;
			if (
				modulePackage.pancake['pancake-module'].sass !== undefined &&
				modulePackage.pancake['pancake-module'].sass.path !== false
			) {
				sassModulePath = Path.normalize(
					`${modulePackage.path}/${modulePackage.pancake['pancake-module'].sass.path}`
				);
			}
			...
			if (!Fs.existsSync(sassModulePath)) {
				...
				Log.verbose(
					`Sass: ${Style.green('⌘')} Found Sass files in ${Style.yellow(
						sassModulePath
					)}`
				);

				//generate the import statements depending on dependencies
				let sass = GenerateSass(
					modulePackage.path,
					modulePackage.name,
					modules,
					GlobalSettings.npmOrg
				);
				allSass += sass; //for SETTINGS.css.name file

				// adding banner and conditional sass-versioning
				if (
					modulePackage.pancake['pancake-module'].sass['sass-versioning'] ===
					true
				) {
		...
		if (modules.length < 1) {
			...
			const locationCSS = Path.normalize(
				`${cwd}/${SETTINGS.css.location}/${SETTINGS.css.name}`
			);
			...
			if (SETTINGS.css.name !== false) {
				compiledAll.push(
					Sassify(locationCSS, SETTINGS.css, allSass).catch((error) => {
						Log.error(error);
					})
				);
			}
			...
			if (SETTINGS.sass.name !== false) {
				const locationSASS = Path.normalize(
					`${cwd}/${SETTINGS.sass.location}/${SETTINGS.sass.name}`
				);
